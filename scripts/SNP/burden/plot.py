...
def plot_mutation_burden(
        ...
        value_col:str="TMB", 
        title:str = None,
        get_sample_id_func:callable = None,
        get_condition_func:callable = None,
        figsize:tuple=(14, 6)
    ):
    """
    Functions: plot mutation burden bar plot with samples colored by culture condition.
    Parameters:
        - df: DataFrame with columns "sample" and value_col (e.g., TMB)
        - outfile: output PNG file path. If None, show the plot instead.
        - value_col: column name in df that contains the mutation burden values to plot.
        - title: title of the plot.
        - get_sample_id_func: function to extract sample ID from the "sample" column. If None, defaults to splitting by "_" and taking the first part.
        - get_condition_func: function to extract culture condition from the "sample" column. If None, defaults to splitting by "_" and taking the last part before "-" as condition.
    Returns:
        None
    Note: x axis will show sample IDs (e.g., GSM4777778) and bars will be colored by culture condition extracted from sample names.
    """
    ...
    if get_condition_func is None:
        get_condition_func = get_condition
    if get_sample_id_func is None:
        get_sample_id_func = get_sample_id

    df = df.copy()
    df["condition"] = df["sample"].apply(get_condition_func)
    df["sample_id"] = df["sample"].apply(get_sample_id_func)
...
if __name__ == "__main__":
    ...
    # )
    df = pd.read_csv("/data/pub/zhousha/20260207_Exome/output/Exome/results/TMB/tmb_results.csv")
    df = df.sort_values("sample").reset_index(drop=True)
    plot_mutation_burden(df,"/data/pub/zhousha/20260207_Exome/output/Exome/results/TMB/tmb_barplot.png", value_col="TMB")
