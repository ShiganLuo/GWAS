...
class pyba_var(object):
    """
    About   : Create a new object class entry, BedAnno::Var,
              parse the variation directly by the ref and alt string.
    Usage   : var = pyba_var( chr, start, end, ref, alt );
           or var = pyba_var( chr, pos, ref, alt );
           or var = pyba_var( varInput_dict );
    ...
    Returns : a new pyba_var instance with the following properties:
                chr    : chr,
    ...
    AAcount = len(AAs)
    AAnumber = dict(zip(list(AAs) + ['?', '.'], range(1, AAcount + 3)))
    CHRORDER = [str(x) for x in range(1,23)] + ['X','Y','MT']
    ...
    def __init__(self, *args):  # chr, start, end, ref, alt
        ...
        if len(args) == 1 and isinstance(args[0], (dict)):
            ...
            if ("chr" not in var_dict
                    or "begin" not in var_dict
                    or "end" not in var_dict and "referenceSequence" not in var_dict):
                raise RuntimeError("unavailable object. need keys: chr, start, alt, ref specified.")
            ...
            if len(args) < 4:
                raise RuntimeError("not enough args, need at least 4 args.")
            elif len(args) == 4:
                chr, start, end, ref = args
            elif len(args) == 5:
                chr, start, end, ref, alt = args

        if end is None or not re.match(r"^\d+$", str(end)):  # from VCF v4.1 1-based start
            if end is not None:
                alt = ref
                ref = end

            ref = pyba_var.normalise_seq(ref)
            alt = pyba_var.normalise_seq(alt)
        ...
        varType, implicit_varType, sm = pyba_var.guess_type(len_ref, ref, alt)

        chr = pyba_var.normalise_chr(chr)
        ...
        if (not varType == 'no-call'
                and not implicit_varType != 'delins'
                and not len_ref > pyba_var.MAX_COMPLEX_PARSING
                and not len(alt) > pyba_var.MAX_COMPLEX_PARSING):
            self.parse_complex()
    ...
    def normalise_seq(seq):
        ...
        if m:
            raise RuntimeError("unrecognized pattern exists,no multiple alts please. [{0}]".format(seq))
        return seq
    ...
    def get_internal(ref, reflen, alt, altlen):
        ...
        Usage   : rephase = get_internal( ref, reflen, alt, altlen );
    ...
    def check_sign(rc):
        ...
        """
        if rc[0] > 0 and [i for i in rc if i < 0] or rc[0] < 0 and [i for i in rc if i > 0]:
            return 0
        return 1
    ...
    def count_content(s):
        s = s.upper()
        l = len(s)
        count = [l] + [0] * pyba_var.AAcount
        for char in s:
            if char not in pyba_var.AAnumber:
                raise RuntimeError("unknown base [{0}]".format(char))
            count[pyba_var.AAnumber[char]] += 1
        return count
    ...
    def check_div(s, rc):
        ...
        """
        rcs = pyba_var.count_content(s)
        if rc[0] % rcs[0] != 0:
            return 0
        div = rc[0] / rcs[0]
        for i in range(1, pyba_var.AAcount + 1):
            if rc[i] != rcs[i] * div:
                return 0
        return div

    def getUnifiedVar(self, strd, norep=False):
        """
        ...
        if "p" in self.__dict__:  # rep
            ...
            if norep:
                if self.rl < self.al:
                    consRef = ''
                    consRL  = 0
                    consAlt = self.rep * (self.alt_cn - self.ref_cn)
                    consAL  = self.al - self.rl
                else:
                    consAlt = ''
                    consAL  = 0
                    consRef = self.rep * (self.ref_cn - self.alt_cn)
                    consRL  = self.rl - self.al
            else:
                consRef = self.r
                consAlt = self.a
                consRL = self.rl
            ...
            consPos = getattr(self, strd)["bp"]
            consRef = getattr(self, strd)["br"]
            consAlt = getattr(self, strd)["ba"]
            consRL = getattr(self, strd)["brl"]
            consAL = getattr(self, strd)["bal"]
    ...
    def get_gHGVS(self):
        ...
        if imp == 'snv':
            ...
            raise RuntimeError("Can not recognize type {0}".format(imp))
        self.gHGVS = gHGVS
    ...
    def parse_complex(self):
        ...
        if get_rst["r"] != len_ref:
            imp_guess, sm = pyba_var.guess_type_by_length(get_rst["r"], get_rst["a"])
            self.imp = imp_guess
            self.sm = sm
            if get_rst["+"] != get_rst["-"]:
                for strd in ("+", "-"):
        ...
        rc_ref = pyba_var.count_content(ref)
        rc_alt = pyba_var.count_content(alt)
        diff = list()
        for i in range(0, pyba_var.AAcount + 1):
            diff.append(rc_ref[i] - rc_alt[i])

        # check if the sign of all base diff are consistent.
        if pyba_var.check_sign(diff):  # possible short tandom repeat variation
            absdiff = map(abs, diff)
            larger, smaller, llen = None, None, None
            if len_ref > len_alt:
                larger = ref
                llen = len_ref
                smaller = alt
            ...
            for rep in range(llen, 0, -1):
                for m in re.finditer(r"([A-Z]+)(?:\1){" + str(rep) + "}", larger):
                    ...
                    cn = pyba_var.check_div(rep_el, absdiff)
                    if cn and pyba_var.check_insrep(larger, smaller, rep_el, cn):
                        lenrep = len(rep_el)
    ...
    def __eq__(self, other):
        if not isinstance(other, pyba_var) or self.chr != other.chr or abs(self.pos - other.pos) > 200:
            return False
        return self.getUnifiedVar('-', True) == other.getUnifiedVar('-', True)
    ...
    def __repr__(self):
        newp, newr, newa, newrl, newal = self.getUnifiedVar('-', True)
        return '{}:{}-{}:{}>{}'.format(self.chr, newp, newp+newrl, newr, newa)
    ...
    def __lt__(self, other):
        try:
            selfChr = CHRORDER.index(self.chr)
        except ValueError:
            selfChr = 100
        try:
            ortherChr = CHRORDER.index(other.chr)
        except ValueError:
            otherChr = 100
        if selfChr < otherChr:
            return True
        elif selfChr == 100 and self.chr < other.chr:
            return True
        elif selfChr > otherChr:
            return False
        else:
