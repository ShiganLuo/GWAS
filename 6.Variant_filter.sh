...
#     -O Splendens_complex615_snp.vcf.gz

# # #为SNP作硬过滤
# java -Xmx200G -jar $GATK VariantFiltration \
#     -V Splendens_complex615_snp.vcf.gz \
#     --filter-expression "QD < 2.0 || MQ < 40.0 || FS > 60.0 || SOR > 3.0 || ReadPosRankSum < -8.0" \
#     --filter-name "SNP_Filter" \
#     -O Splendens_complex615_snp.hardfilter.vcf.gz

# # #使用SelectVariants，选出Indel
# java -Xmx200G -jar $GATK SelectVariants \
#     -select-type INDEL \
#     -V ${outdir}/GVCF/chr_gz/Betta.merge.vcf.gz \
#     -O Splendens_complex615_indel.vcf.gz

# #为Indel作过滤
# java -Xmx200G -jar $GATK VariantFiltration \
#     -V Splendens_complex615_indel.vcf.gz \
#     --filter-expression "QD < 2.0 || FS > 200.0 || SOR > 10.0 || MQRankSum < -12.5 || ReadPosRankSum < -8.0" \
#     --filter-name "INDEL_Filter" \
#     -O Splendens_complex615_indel.hardfilter.vcf.gz

# # 重新合并过滤后的SNP和Indel
# java -Xmx200G -jar $GATK MergeVcfs \
#    -I Splendens_complex615_snp.hardfilter.vcf.gz \
#    -I Splendens_complex615_indel.hardfilter.vcf.gz \
#    -O Splendens_complex615_all.hardfilter.vcf.gz

# java -Xmx80G -jar $GATK SelectVariants \
# 	--select "vc.isNotFiltered()" \
# 	-R $RefSeq \
# 	-V Splendens_complex615_all.hardfilter.vcf.gz \
# 	-O Splendens_complex615_all.hardfiltered.vcf.gz
